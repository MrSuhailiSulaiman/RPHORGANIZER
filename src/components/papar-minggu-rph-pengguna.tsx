...
export function PaparMingguRphPengguna({ penggunaId }: { penggunaId: string }) {
  ...
  useEffect(() => {
    ...
    async function muat() {
      setSedangMuat(true);
      try {
        const res = await fetch(`/api/pengguna/${penggunaId}?t=${Date.now()}`, { cache: "no-store" });
        ...
        let penuh: RphRekod[] = [];
        if (idMinggu.length) {
          const penuhRes = await fetch(
            `/api/pengguna/${penggunaId}?ids=${encodeURIComponent(idMinggu.join(","))}`,
            { cache: "no-store" }
          );
  ...
  async function muatPdfMinggu() {
    ...
    const idMinggu = (kumpul?.item.map((item) => item.id) ?? borang.map((item) => item.id ?? "")).filter(
      Boolean
    );
    if (!idMinggu.length) {
  ...
  const nomborMinggu = minggu || kumpulanMinggu[0]?.minggu || "?";
      ...
      <div className="sticky top-0 z-20 -mx-4 flex flex-wrap items-center justify-between gap-3 border-b bg-background/95 px-4 py-3 backdrop-blur">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">{borang.length} sesi · lihat sahaja</p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <TapisMingguRph
            ...
            onChange={(pilih) => {
              ...
              if (!kumpul) return;
              router.push(
                `/pengguna/${penggunaId}/minggu?minggu=${pilih}&ids=${kumpul.item.map((item) => item.id).join(",")}`
              );
            }}
          />
          <Button
        ...
        borang.map((item, indeks) => (
          <section key={item.id ?? `sesi-${indeks}`} className="space-y-2" id={`sesi-${indeks + 1}`}>
            ...
            <fieldset disabled className="min-w-0">
              <JadualRph borang={item} onChange={() => undefined} />
            </fieldset>
          </section>
        ))
      ) : (
