...
export function SenaraiPengguna() {
  ...
  const muatSemula = useCallback(async () => {
    const res = await fetch(`/api/pengguna?t=${Date.now()}`, { cache: "no-store" });
    const json = await res.json();
    if (!res.ok) throw new Error(json.ralat ?? "Gagal memuatkan senarai pengguna.");
    setPengguna((json.pengguna ?? []) as RekodPenggunaSenarai[]);
  }, []);
  ...
  return (
    <div className="space-y-6">
      <BorangDaftar
        onSelesai={() => {
          void muatSemula().catch((error) =>
            toast.error(error instanceof Error ? error.message : "Gagal memuatkan.")
          );
        }}
      />
      {sedangMuat ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center py-16 text-center">
            ...
            <p className="mt-1 max-w-md text-sm text-muted-foreground">
              Daftar pengguna biasa di atas. Akaun baharu akan dipaparkan di sini.
            </p>
          </CardContent>
        </Card>
      ) : (
